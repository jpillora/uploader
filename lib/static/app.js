  ...
  methods: {
    ...
    dequeue() {
      ...
      xhr.addEventListener("load", () => {
        job.status = xhr.status;
        job.message = xhr.responseText;
        this.dequeue();
      });
      const fd = new FormData();
      fd.append("file", job.file);
      xhr.send(fd);
      this.handled.push(job);
    },
  },

  template: /*html*/ `
    <!-- GITHUB -->
    <a href="https://github.com/jpillora/uploader">
      <img style="opacity: 0.5; position: absolute; z-index: 2; top: 0; left: 0; border: 0;" src="${gh}" alt="Fork me on GitHub">
    </a>
    <!-- MESSAGE -->
    <div style="display: flex; height: 100%; justify-content: center; align-items: center;">
      <div style="font-family: Charter, 'Bitstream Charter', 'Sitka Text', Cambria, serif; text-align: center; padding: 30px 10px;">
        <div>
          <img src="logo.svg" width="100" height="100" alt="Uploader">
        </div>
        <h1 style="text-decoration: underline;">Upload files</h1>
        <div v-if="handled.length == 0">
          <h2>Drag and drop</h2>
          <div>or</div>
          <h2>Click to select</h2>
        </div>
        <div v-else>
          <div v-for="job in handled"
            :style="{
              width: 280,
            ...
            <div :style="{
              ...
              alignItems: 'center',
              overflow: 'hidden',
            }"><span>{{ job.progress }}%</span></div>
          </div>
        </div>
      </div>
    </div>
    <!-- FILE CHOOSER -->
    <input style="display: none;" type="file" ref="chooser" multiple @change="choose"/>
    <!-- DROPPER -->
    <div
      :style="{
      ...
      v-on:dragenter.prevent="dragging = true"
      v-on:dragover.prevent
      v-on:dragleave.prevent="dragging = false"
      v-on:drop.prevent="drop($event); dragging = false"
      v-on:click="$refs.chooser.click()"
    ></div>
  `,
});
